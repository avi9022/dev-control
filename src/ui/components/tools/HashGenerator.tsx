...
function generateMd5(text: string): string {
  ...
  function md51(s: string) {
    ...
    let i: number; for (i = 64; i <= n; i += 64) { md5cycle(state, md5blk(s.substring(i - 64, i))) }
  ...
  function hex(x: number[]) {
    ...
    for (let i = 0; i < x.length; i++) { for (let j = 0; j < 4; j++) { s += chars.charAt((x[i] >> (j * 8 + 4)) & 0x0F) + chars.charAt((x[i] >> (j * 8)) & 0x0F) } }
    return s
  }
...
export const HashGenerator: FC = () => {
  ...
  useEffect(() => {
    ...
    const computeHashes = async () => {
      const results: HashResult[] = [{ algorithm: 'MD5', hash: generateMd5(input) }]
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          {hashes.map(({ algorithm, hash }) => (
            <div key={algorithm} className="bg-muted/50 p-3 rounded-md">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm font-semibold">{algorithm}</span>
                <CopyButton text={hash} />
              </div>
              <code className="text-xs font-mono break-all select-all">{hash}</code>
            </div>
          ))}
        </div>
