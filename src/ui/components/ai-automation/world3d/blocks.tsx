...
function getMaterials(type: BlockType, color?: string): THREE.MeshStandardMaterial[] {
  ...
  if (!materialCache.has(key)) {
    ...
    const isTransparent = type === 'water' || type === 'bars'

    const sideMat = new THREE.MeshStandardMaterial({ map: textures.side, transparent: isTransparent, alphaTest: type === 'bars' ? 0.1 : 0, opacity: type === 'water' ? 0.6 : 1 })
    const topMat = new THREE.MeshStandardMaterial({ map: textures.top, transparent: isTransparent, alphaTest: type === 'bars' ? 0.1 : 0, opacity: type === 'water' ? 0.5 : 1 })
    const bottomMat = new THREE.MeshStandardMaterial({ map: textures.bottom, transparent: isTransparent, alphaTest: type === 'bars' ? 0.1 : 0, opacity: type === 'water' ? 0.7 : 1 })
