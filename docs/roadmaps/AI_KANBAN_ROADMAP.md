...
- [x] **F36**: AI Task Planner — user describes a goal in natural language, an agent analyzes the codebase and breaks it down into multiple tasks with titles, descriptions, project assignments, and suggested pipeline phases, auto-creating them in the backlog
- [x] **F42**: 3D Task Visualization — fun alternative view that renders tasks as characters in a Minecraft-style voxel world, where agents visually perform their work (planning agent reads blueprints, worker agent builds structures, reviewer agent inspects buildings), with task progress reflected as construction progress and phase transitions as movement between zones (sub-roadmap: `docs/F42_3D_VISUALIZATION_ROADMAP.md`)
- [x] **F43**: Code Quality & Linting Rules — establish and enforce project-wide coding standards with ESLint rules, run a full codebase quality assessment, fix inconsistencies (naming, patterns, dead code, error handling), and document conventions in CLAUDE.md
- [x] **F44**: Codebase Audit & Refactor — comprehensive review of the entire codebase for code quality, performance, accessibility, and maintainability; identify tech debt, refactor large files, extract shared utilities, and ensure consistent patterns across all modules

## Planned Features

- [ ] **F62**: Task Clusters — parent task with ordered subtasks that share a worktree and branch. One kanban card per cluster with expand overlay to see subtasks. Each subtask runs the full pipeline, auto-advances on completion. Incremental diffs per subtask via commit baseline tracking. Cluster context injected into agent prompts. Planner creates clusters via `create_cluster` MCP tool. (Design: `docs/plans/2026-03-28-task-clusters-design.md`)
...
- [ ] **B7**: Edit button visible after task started — the Edit button in task detail header should only be shown when the task has never left BACKLOG (no pipeline phases in history). Once a task has entered any agent/manual phase, editing should be disabled since worktrees and agent context already depend on the original task data.
- [ ] **B8**: Smart default tab on task open — when opening a task, default to: Agents tab if an agent is running, Changes tab if there are uncommitted changes but no agent running, Task tab otherwise. Currently always defaults based on manual phase check.
- [ ] **B9**: Phase timeline truncation — the PhaseStepper truncates to show first 2 + last 3 entries when there are 10+ phases. Remove the truncation and add horizontal scrolling instead so all phases are always visible.
